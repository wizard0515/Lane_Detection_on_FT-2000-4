	...
	for (int steplength = 2; steplength <= lim; steplength *= 2)
	{
		for (int step = 0; step < lim / steplength; step++)
		{
			for (int i = 0; i < steplength / 2; i++)
			{
				...
				temp[0] = output.at<cv::Vec2f>(0, Index1)[0] * WN.at<cv::Vec2f>(0, (long)i * lim / steplength)[0] - output.at<cv::Vec2f>(0, Index1)[1] * WN.at<cv::Vec2f>(0, (long)i * lim / steplength)[1];
				temp[1] = output.at<cv::Vec2f>(0, Index1)[0] * WN.at<cv::Vec2f>(0, (long)i * lim / steplength)[1] + output.at<cv::Vec2f>(0, Index1)[1] * WN.at<cv::Vec2f>(0, (long)i * lim / steplength)[0];
